...
if aw == 'y':
    ...
    for i in range(n):
        ...
        if pk == 'com.quvideo.slideplus':
            """filedir = os.path.exists("D:\\git\\monkeyTest\\")
            if filedir:
                print("File Exist,go on testing!")
            else:
                os.mkdir("D:\\git\\monkeyTest\\")"""
            devicedir = os.path.exists("D:\\git\\monkeyTest\\" + name + '-' + model + '-' + dev)
            if devicedir:
                print("File Exist, go on testing!")
            else:
                os.mkdir("D:\\git\\monkeyTest\\" + name + '-' + model + '-' + dev)  # 按设备ID生成日志目录文件夹
            wl = open("D:\\git\\monkeyTest\\" + name + '-' + model + '-' + dev + '-logcat' + '.cmd', 'w')
            wl.write('adb -s ' + dev + ' logcat' + ' > D:\\git\\monkeyTest\\' + '"' + name + '-' + model + '-' + dev + '"' + '\\'+ run_time + '_logcat.txt\n' )
            #wl.write(' > D:\\git\\monkeyTest\\' + '"' + name + '-' + model + '-' + dev + '"' + '\\logcat.txt\n')
            wl.close()
            #if testmodel == '1':
            wd = open("D:\\git\\monkeyTest\\" + name + '-' + model + '-' + dev + '-device' + '.cmd', 'w')
            wd.write(
                'adb -s ' + dev + ' shell monkey -p com.quvideo.slideplus -s 200 --throttle 500 --ignore-crashes --ignore-timeouts --pct-touch 45 --pct-trackball 15 --pct-appswitch 10 --pct-syskeys 10 --pct-motion 20 -v -v 210000 ')  # 选择设备执行monkey
            wd.write('> D:\\git\\monkeyTest\\' + '"' + name + '-' + model + '-' + dev + '"' + '\\' + run_time + '_monkey.txt\n')
            ...
            """elif testmodel == '2':
                ...
                wd.write(
                    '\nadb -s ' + dev + ' shell monkey -p com.quvideo.slideplus -s 200 --throttle 500 --ignore-crashes --ignore-timeouts --pct-touch 45 --pct-trackball 15 --pct-appswitch 10 --pct-syskeys 10 --pct-motion 20 -v -v 210000 ')  # 选择设备执行monkey
                wd.write('> D:\\git\\monkeyTest\\' + '"' + name + '-' + model + '-' + dev + '"' + '\\monkey.txt\n')
